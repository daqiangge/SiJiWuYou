...
@implementation OrderAfterSalesCell
 ...
 * 一
 *****************************************************************************************/
@interface OrderAfterSalesFirstCell ()

@property (nullable, nonatomic, weak) IBOutlet UILabel *belongNameLabel;

@end

@implementation OrderAfterSalesFirstCell
 ...
 * 二
 *****************************************************************************************/
@interface OrderAfterSalesSecondCell ()
...
@implementation OrderAfterSalesSecondCell
 ...
 * 三
 *****************************************************************************************/
@interface OrderAfterSalesThirdCell () <
...
@property (nullable, nonatomic, weak) IBOutlet UILabel *dateLabel;
...
@property (nullable, nonatomic, weak) IBOutlet OrderAfterSalesThirdV *firstV;
@property (nullable, nonatomic, weak) IBOutlet OrderAfterSalesThirdV *secondV;
@property (nullable, nonatomic, weak) IBOutlet OrderAfterSalesThirdV *thirdV;
@property (nullable, nonatomic, weak) IBOutlet OrderAfterSalesThirdV *fourthV;
@property (nullable, nonatomic, weak) IBOutlet OrderAfterSalesThirdV *fifthV;

@property (nullable, nonatomic, strong) OrderAfterSalesVM *viewModel;
@property (nullable, nonatomic, strong) NSArray<OrderAfterSalesThirdV *> *vArray;

@end

@implementation OrderAfterSalesThirdCell
...
- (void)bindViewModel:(OrderAfterSalesVM *)viewModel {
    ...
    @weakify(self)
    [RACObserve(_viewModel, serviceAddress) subscribeNext:^(ReceiptAddressItemM *address) {
        @strongify(self)
        if (address) {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    for (int i = 0; i < _viewModel.imageMutableArray.count; i++) {
        OrderAfterSalesThirdV *v = self.vArray[i];
        v.logoImage = _viewModel.imageMutableArray[i];
    }
    if (_viewModel.imageMutableArray.count < self.vArray.count) {
        OrderAfterSalesThirdV *v = self.vArray[_viewModel.imageMutableArray.count];
        v.hidden = NO;
    }
    //
...
- (MLPhotoBrowserPhoto *) photoBrowser:(MLPhotoBrowserViewController *)browser photoAtIndexPath:(NSIndexPath *)indexPath {
    ...
    OrderAfterSalesThirdV *v = self.vArray[indexPath.row];
...
- (BOOL)photoBrowser:(MLPhotoBrowserViewController *)photoBrowser willRemovePhotoAtIndexPath:(NSIndexPath *)indexPath {
    ...
    OrderAfterSalesThirdV *v = self.vArray[0];
    v.logoImage = nil;
...
- (IBAction)serviceDate:(id)sender {
    @weakify(self)
    RMAction *selectAction = [RMAction
                              ...
                              andHandler:^(RMActionController *controller) {
                                  @strongify(self)
                                  NSDate *tempDate = ((UIDatePicker *)controller.contentView).date;
                                  _dateLabel.text = [self.viewModel.dateFormatter stringFromDate:tempDate];
                                  self.viewModel.serviceDate = [self.viewModel.dateFormatter stringFromDate:tempDate];
                              }];
    ...
    [_viewModel.masterVC presentViewController:dateSelectionController
                                      animated:YES
                                    completion:nil];
}

- (IBAction)serviceAddress:(id)sender {
    ...
    receiptAddressVC.selectAddress = ^(ReceiptAddressItemM *model){
        @strongify(self)
        self.viewModel.serviceAddress = model;
    };
    [_viewModel.masterVC.navigationController pushViewController:receiptAddressVC
                                                        animated:YES];
}
...
- (NSArray<OrderAfterSalesThirdV *> *)vArray {
    if (!_vArray) {
        _vArray = @[
                    _firstV,
                    _secondV,
...
@implementation OrderAfterSalesThirdV
